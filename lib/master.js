...
module.exports = {
    ...
    methods: {
        ...
        async storeHashes(hashes) {
            let query = `UPDATE ${process.env.CASSANDRA_KEYSPACE}.${this.hashTable} SET hashes = :hashes WHERE id = 1;`;
            let params = { 
                hashes            };
            try {
        ...
        async readHashes() {
            let query = `SELECT hashes FROM ${process.env.CASSANDRA_KEYSPACE}.${this.hashTable} WHERE id = 1;`;
            let params = { };
            try {
        ...
        async checkKeys() {
            let query = `SELECT owner FROM ${process.env.CASSANDRA_KEYSPACE}.${this.keyTable} LIMIT 1;`;
            let params = { };
            try {
                let result = await this.cassandra.execute(query, params, {prepare: true});
                let row = result.first();
        ...
        async connect () {
            ...
            let query = `CREATE TABLE IF NOT EXISTS ${process.env.CASSANDRA_KEYSPACE}.${this.hashTable} `;
